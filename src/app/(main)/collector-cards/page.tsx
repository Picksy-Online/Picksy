...
export default function CollectorCardsPage() {
  ...
  const sortedAndFilteredProducts = useMemo(() => {
    let filteredProducts = allProducts.filter(p => !p.isPrivate && p.category === 'Collector Cards');
  ...
  const availableSellers = useMemo(() => {
    ...
    const cardSellerIds = new Set(allProducts.filter(p => p.category === 'Collector Cards').map(p => p.sellerId));
    return users.filter(seller => !blockedIds.includes(seller.id) && cardSellerIds.has(seller.id));
  }, [user]);

  return (
    <div className="pt-5 pb-12 p-5">
      <h1 className="text-3xl font-bold font-headline">Collector Cards</h1>
      <p className="mt-2 text-muted-foreground">Browse our collection of trading and collector cards.</p>
