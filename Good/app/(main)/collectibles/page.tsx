...
const PAGE_SIZE = 24;
const COLLECTIBLE_CATEGORIES = ['Vintage'];
...
export default function CollectiblesPage() {
  ...
  const handleFilterChange = (key: string, value: string | string[] | null) => {
    ...
    const newQuery = createQueryString({ [key]: valueStr, page: '1' });
  ...
  const loadMoreProducts = useCallback(async () => {
    ...
    try {
      const { products: newProducts, hasMore: newHasMore } = await getProducts({
        ...initialSearchParams,
        category: COLLECTIBLE_CATEGORIES.join(','),
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-2xl lg:text-3xl font-bold font-headline text-foreground">Collectibles</h1>
            </div>
          </div>
